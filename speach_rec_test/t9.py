...
MAX_ACCUMULATION_SECONDS = 5 # Maximum buffer window 
SILENCE_TIMEOUT_SECONDS = 2
AUDIO_PROCESS_INTERVAL = 0.05  # How often to check for new audio data

# Global list to store transcribed text segments
transcribed_text = ""
transcribed_segments = []
last_transcription_time = None
silence_buffer = np.array([]) #Store the start times of silence audio
...
def process_audio_stream(model, q):
    ...
    global transcribed_segments, last_transcription_time, running, silence_buffer
    
    while running:
        has_transcribed = False #reset this each loop
        try:
        ...
        if len(audio_buffer_list) > 0: #If we have audio data to transcribe
            ...
            #Concatenate the batches only when we are about to transcribe
            ...
            try:
                ...
                if has_transcribed:
                  ...
                  silence_buffer = np.array([]) #Clear silence buffer
        ...
        if len(audio_buffer_list) > 0 and not has_transcribed:
           silence_buffer = np.append(silence_buffer, time.time())
           if len(silence_buffer) > 1:
               if (silence_buffer[-1] - silence_buffer[0]) >= SILENCE_TIMEOUT_SECONDS:
                 logging.info("No transcription detected for 2 seconds, terminating.")
                 running = False
            
        time.sleep(AUDIO_PROCESS_INTERVAL)
